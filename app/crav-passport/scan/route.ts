...
/**
 * GET /crav-passport/scan?t=<jwt>
 *
 * Universal landing target for every CRAV Passport QR code — both the
 ...
 * 3. Verifies the token signature. Invalid → /crav-passport?error=invalid.
 * 4. Looks up the shop. Unknown id → /crav-passport?error=unknown.
 * 5. Adds the sticker to the user's passport (idempotent per the
 *    lifetime-uniqueness rule) and redirects to
 *    /crav-passport?collected=<shopId>(&new=1).
 *
...
export async function GET(req: NextRequest) {
  ...
  if (!token) {
    return NextResponse.redirect(new URL("/crav-passport?error=missing", req.url));
  }

  const session = await getSession();
  if (!session) {
    // Preserve the full scan URL so the user lands back here after sign-in.
    const next = `/crav-passport/scan?t=${encodeURIComponent(token)}`;
  ...
  if (!payload) {
    return NextResponse.redirect(new URL("/crav-passport?error=invalid", req.url));
  }

  const shop = dispensaries.find((d) => d.id === payload.shopId);
  if (!shop) {
    return NextResponse.redirect(new URL("/crav-passport?error=unknown", req.url));
  }
  ...
  const target = new URL("/crav-passport", req.url);
